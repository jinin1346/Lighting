...
<body>
	...
	<form method="POST" action="/lighting/meeting/updateok.do">
	<div class="container">
		<!-- 상단 섹션 -->
		<div class="top-section">
			<div class="left-section">
                ...
                <input type="text" id="title" placeholder="제목은 20자 이내로 입력 가능합니다." maxlength="20" required name="title" value="${postdto.title}">

				<div class="time-section">
					<input type="date" id="date" required name="date" value="${date}">
					<input type="time" id="time" required name="time" value="${time}">
					<input type="number" id="people" placeholder="인원수" min="2" max="10" required name="capacity" value="${postdto.capacity}">
				</div>
			</div>
		</div>
		...
		<div class="middle-section">
			<!-- 왼쪽 중간: 글 내용 텍스트 영역 -->
			<div class="left-section">
				<textarea id="content" placeholder="글 내용을 입력하세요." maxlength="2000" name="content">${content}</textarea>
			</div>
			...
			<div>
				...
				<div class="bottons-box">
					...
					<div class="buttons">
						...
						<button id="attend" type="submit">글 수정하기 </button>
					</div>
				</div>
			</div>
		</div>
...
<%@ include file="/WEB-INF/views/inc/footer.jsp" %>
    ...
    <script>
        ...
        if ("${postdto.latitude}" == null || "${postdto.latitude}" == "") {
    		...
    		let latitude = "${postdto.latitude}";
			let longitude = "${postdto.longitude}";
        ...
        $('#categoryMain option[value="${categorydto.tblCategoryMainSeq}"]').attr('selected', 'selected');
		
        $.ajax({
            url: '/lighting/meeting/getcategorysub.do',
            type: 'GET',
            data: 'tblCategoryMainSeq=' + ${categorydto.tblCategoryMainSeq},
            dataType: 'json',
            success: function(result) {
				...
				$('#categorySub option[value="${postdto.tblCategorySubSeq}"]').attr('selected', 'selected');
